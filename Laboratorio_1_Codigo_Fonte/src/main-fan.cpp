...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //                |          |  |                 |
        //                V          V  V                 V
        glDrawElements(GL_TRIANGLE_FAN, VERTICES_EXTERNOS + 2, GL_UNSIGNED_BYTE, 0);
...
GLuint BuildTriangles()
{
    ...
    //Constante M_PI vem da math.h, que que representa meio círculo
    //Precisamos de Radianos 2 * M_PI para ter o círculo inteiro
    float Rad2Pi = M_PI * 2;
    GLfloat NDC_coefficients[4 * (VERTICES_EXTERNOS + 1)];

    float angulo = 0;
    int i = 0;

    for(i = 0; i <= VERTICES_EXTERNOS * 4; i += 4){
        if(i == 0) {
            NDC_coefficients[i + 0] = 0.0f;
            NDC_coefficients[i + 1] = 0.0f;
            NDC_coefficients[i + 2] = 0.0f;
            NDC_coefficients[i + 3] = 1.0f;
        } else {
            NDC_coefficients[i + 0] = RAIO * cos(angulo);
            NDC_coefficients[i + 1] = RAIO * sin(angulo);
            NDC_coefficients[i + 2] = 0.0f;
            NDC_coefficients[i + 3] = 1.0f;
        }
    ...
    GLfloat color_coefficients[4 * (VERTICES_EXTERNOS + 1)];

    for(i = 0; i <= (VERTICES_EXTERNOS + 1) * 4; i += 4){
        if(i == 0) {
    ...
    GLubyte indices[VERTICES_EXTERNOS + 2];
    for(i = 0; i < VERTICES_EXTERNOS; i++){
        indices[i] = i;
    }
    indices[VERTICES_EXTERNOS] = VERTICES_EXTERNOS;
    indices[VERTICES_EXTERNOS + 1] = indices[1];
